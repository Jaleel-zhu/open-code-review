...
- 如果用户提供了 `--commit` 或 `-c`：原样传递。
- 如果用户提供了 `--from` 和 `--to`：原样传递。
- （可选）提供 `--background "需求上下文"` 以审查需求是否被正确实现。
- 捕获完整 stdout，设置 5 分钟超时。
...
- **高**：明显的 bug、安全问题、明确的错误，或提出了精准修复方案的建议
...
然后在 Claude Code 中使用 `/open-code-review` 即可运行审查。该斜杠命令会运行 OCR、过滤评估每条评论，并自动修复值得采纳的问题。

#### Cursor / Windsurf

将以下指令添加到你的 `.cursorrules` 或 `.windsurfrules` 文件中：
...
如果未找到 `ocr` 命令，运行安装：npm i -g @alibaba-group/open-code-review
```
...
`--audience agent` 标志会抑制进度 UI，输出简洁的结构化摘要。`--background` 标志提供需求上下文，让 OCR 可以验证实现是否符合意图。
